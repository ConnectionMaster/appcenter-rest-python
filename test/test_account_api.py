# coding: utf-8

"""
    App Center Client

    Microsoft Visual Studio App Center API  # noqa: E501

    OpenAPI spec version: preview
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""

from __future__ import absolute_import

import unittest

import appcenter
from api.account_api import AccountApi  # noqa: E501
from appcenter.rest import ApiException


class TestAccountApi(unittest.TestCase):
    """AccountApi unit test stubs"""

    def setUp(self):
        self.api = api.account_api.AccountApi()  # noqa: E501

    def tearDown(self):
        pass

    def test_aad_group_list_for_org(self):
        """Test case for aad_group_list_for_org

        """
        pass

    def test_api_tokens_delete(self):
        """Test case for api_tokens_delete

        """
        pass

    def test_api_tokens_list(self):
        """Test case for api_tokens_list

        """
        pass

    def test_api_tokens_new(self):
        """Test case for api_tokens_new

        """
        pass

    def test_app_invitations_accept(self):
        """Test case for app_invitations_accept

        """
        pass

    def test_app_invitations_create(self):
        """Test case for app_invitations_create

        """
        pass

    def test_app_invitations_create_by_email(self):
        """Test case for app_invitations_create_by_email

        """
        pass

    def test_app_invitations_delete(self):
        """Test case for app_invitations_delete

        """
        pass

    def test_app_invitations_list(self):
        """Test case for app_invitations_list

        """
        pass

    def test_app_invitations_reject(self):
        """Test case for app_invitations_reject

        """
        pass

    def test_app_invitations_update_permissions(self):
        """Test case for app_invitations_update_permissions

        """
        pass

    def test_apps_create(self):
        """Test case for apps_create

        """
        pass

    def test_apps_create_for_org(self):
        """Test case for apps_create_for_org

        """
        pass

    def test_apps_delete(self):
        """Test case for apps_delete

        """
        pass

    def test_apps_delete_avatar(self):
        """Test case for apps_delete_avatar

        """
        pass

    def test_apps_get(self):
        """Test case for apps_get

        """
        pass

    def test_apps_get_for_org_user(self):
        """Test case for apps_get_for_org_user

        """
        pass

    def test_apps_get_teams(self):
        """Test case for apps_get_teams

        """
        pass

    def test_apps_list(self):
        """Test case for apps_list

        """
        pass

    def test_apps_list_for_org(self):
        """Test case for apps_list_for_org

        """
        pass

    def test_apps_list_testers(self):
        """Test case for apps_list_testers

        """
        pass

    def test_apps_remove_user(self):
        """Test case for apps_remove_user

        """
        pass

    def test_apps_transfer_ownership(self):
        """Test case for apps_transfer_ownership

        """
        pass

    def test_apps_transfer_to_org(self):
        """Test case for apps_transfer_to_org

        """
        pass

    def test_apps_update(self):
        """Test case for apps_update

        """
        pass

    def test_apps_update_avatar(self):
        """Test case for apps_update_avatar

        """
        pass

    def test_apps_update_user_permissions(self):
        """Test case for apps_update_user_permissions

        """
        pass

    def test_azure_subscription_delete_for_app(self):
        """Test case for azure_subscription_delete_for_app

        """
        pass

    def test_azure_subscription_link_for_app(self):
        """Test case for azure_subscription_link_for_app

        """
        pass

    def test_azure_subscription_list_for_app(self):
        """Test case for azure_subscription_list_for_app

        """
        pass

    def test_azure_subscription_list_for_org(self):
        """Test case for azure_subscription_list_for_org

        """
        pass

    def test_azure_subscription_list_for_user(self):
        """Test case for azure_subscription_list_for_user

        """
        pass

    def test_distribution_group_invitations_accept_all(self):
        """Test case for distribution_group_invitations_accept_all

        """
        pass

    def test_distribution_groups_add_apps(self):
        """Test case for distribution_groups_add_apps

        """
        pass

    def test_distribution_groups_add_user(self):
        """Test case for distribution_groups_add_user

        """
        pass

    def test_distribution_groups_add_users_for_org(self):
        """Test case for distribution_groups_add_users_for_org

        """
        pass

    def test_distribution_groups_bulk_delete_apps(self):
        """Test case for distribution_groups_bulk_delete_apps

        """
        pass

    def test_distribution_groups_bulk_delete_users(self):
        """Test case for distribution_groups_bulk_delete_users

        """
        pass

    def test_distribution_groups_create(self):
        """Test case for distribution_groups_create

        """
        pass

    def test_distribution_groups_create_for_org(self):
        """Test case for distribution_groups_create_for_org

        """
        pass

    def test_distribution_groups_delete(self):
        """Test case for distribution_groups_delete

        """
        pass

    def test_distribution_groups_delete_for_org(self):
        """Test case for distribution_groups_delete_for_org

        """
        pass

    def test_distribution_groups_details_for_org(self):
        """Test case for distribution_groups_details_for_org

        """
        pass

    def test_distribution_groups_get(self):
        """Test case for distribution_groups_get

        """
        pass

    def test_distribution_groups_get_apps(self):
        """Test case for distribution_groups_get_apps

        """
        pass

    def test_distribution_groups_get_for_org(self):
        """Test case for distribution_groups_get_for_org

        """
        pass

    def test_distribution_groups_list(self):
        """Test case for distribution_groups_list

        """
        pass

    def test_distribution_groups_list_all_testers_for_org(self):
        """Test case for distribution_groups_list_all_testers_for_org

        """
        pass

    def test_distribution_groups_list_for_org(self):
        """Test case for distribution_groups_list_for_org

        """
        pass

    def test_distribution_groups_list_users(self):
        """Test case for distribution_groups_list_users

        """
        pass

    def test_distribution_groups_list_users_for_org(self):
        """Test case for distribution_groups_list_users_for_org

        """
        pass

    def test_distribution_groups_patch_for_org(self):
        """Test case for distribution_groups_patch_for_org

        """
        pass

    def test_distribution_groups_remove_user(self):
        """Test case for distribution_groups_remove_user

        """
        pass

    def test_distribution_groups_resend_invite(self):
        """Test case for distribution_groups_resend_invite

        """
        pass

    def test_distribution_groups_resend_shared_invite(self):
        """Test case for distribution_groups_resend_shared_invite

        """
        pass

    def test_distribution_groups_update(self):
        """Test case for distribution_groups_update

        """
        pass

    def test_invitations_sent(self):
        """Test case for invitations_sent

        """
        pass

    def test_org_invitations(self):
        """Test case for org_invitations

        """
        pass

    def test_org_invitations_accept(self):
        """Test case for org_invitations_accept

        """
        pass

    def test_org_invitations_create(self):
        """Test case for org_invitations_create

        """
        pass

    def test_org_invitations_delete(self):
        """Test case for org_invitations_delete

        """
        pass

    def test_org_invitations_list_pending(self):
        """Test case for org_invitations_list_pending

        """
        pass

    def test_org_invitations_reject(self):
        """Test case for org_invitations_reject

        """
        pass

    def test_org_invitations_send_new_invitation(self):
        """Test case for org_invitations_send_new_invitation

        """
        pass

    def test_org_invitations_update(self):
        """Test case for org_invitations_update

        """
        pass

    def test_organization_add_aad_groups(self):
        """Test case for organization_add_aad_groups

        """
        pass

    def test_organization_delete_aad_group(self):
        """Test case for organization_delete_aad_group

        """
        pass

    def test_organization_delete_avatar(self):
        """Test case for organization_delete_avatar

        """
        pass

    def test_organization_update_aad_groups(self):
        """Test case for organization_update_aad_groups

        """
        pass

    def test_organization_update_avatar(self):
        """Test case for organization_update_avatar

        """
        pass

    def test_organizations_create_or_update(self):
        """Test case for organizations_create_or_update

        """
        pass

    def test_organizations_delete(self):
        """Test case for organizations_delete

        """
        pass

    def test_organizations_get(self):
        """Test case for organizations_get

        """
        pass

    def test_organizations_list(self):
        """Test case for organizations_list

        """
        pass

    def test_organizations_list_administered(self):
        """Test case for organizations_list_administered

        """
        pass

    def test_organizations_update(self):
        """Test case for organizations_update

        """
        pass

    def test_sharedconnection_connections(self):
        """Test case for sharedconnection_connections

        """
        pass

    def test_teams_add_app(self):
        """Test case for teams_add_app

        """
        pass

    def test_teams_add_user(self):
        """Test case for teams_add_user

        """
        pass

    def test_teams_create_team(self):
        """Test case for teams_create_team

        """
        pass

    def test_teams_delete(self):
        """Test case for teams_delete

        """
        pass

    def test_teams_get_team(self):
        """Test case for teams_get_team

        """
        pass

    def test_teams_get_users(self):
        """Test case for teams_get_users

        """
        pass

    def test_teams_list_all(self):
        """Test case for teams_list_all

        """
        pass

    def test_teams_list_apps(self):
        """Test case for teams_list_apps

        """
        pass

    def test_teams_remove_app(self):
        """Test case for teams_remove_app

        """
        pass

    def test_teams_remove_user(self):
        """Test case for teams_remove_user

        """
        pass

    def test_teams_update(self):
        """Test case for teams_update

        """
        pass

    def test_teams_update_permissions(self):
        """Test case for teams_update_permissions

        """
        pass

    def test_users_get(self):
        """Test case for users_get

        """
        pass

    def test_users_get_for_org(self):
        """Test case for users_get_for_org

        """
        pass

    def test_users_list(self):
        """Test case for users_list

        """
        pass

    def test_users_list_for_org(self):
        """Test case for users_list_for_org

        """
        pass

    def test_users_remove_from_org(self):
        """Test case for users_remove_from_org

        """
        pass

    def test_users_update(self):
        """Test case for users_update

        """
        pass

    def test_users_update_org_role(self):
        """Test case for users_update_org_role

        """
        pass


if __name__ == '__main__':
    unittest.main()
